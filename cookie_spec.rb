...
describe Cookie do
  ...
  describe "#bake!" do
    it "requires an integer time argument" do
      [nil, "foo", 14.5, Date.new].each do |arg|
        expect { cookie.bake!(arg) }.to raise_error(ArgumentError)
      end
      expect { cookie.bake!(5) }.to_not raise_error
    end

    it "returns the cookie object" do
      expect(cookie.bake!(2)).to be_an_instance_of Cookie
    end
  ...
  describe "#status" do
    it "returns the cookie's current status" do
      expect(Cookie.new(type: 'Caramel', time_baked: 12).bake!(12).status).to eq(:burned)
    end
    ...
    context "when baked for less than 7 minutes" do
      it "is `:doughy`" do
      expect(Cookie.new(type: 'Raisin').bake!(1).status).to eq(:doughy)
      end
    end  

    context "when baked for at least 7 but less than 10 minutes" do
      it "is `:almost_ready`" do
        expect(Cookie.new(type: 'Raisin').bake!(8).status).to eq(:almost_ready)
      end
    end

    context "when baked for at least 10 but less than 12 minutes" do
      it "is `:ready`" do
        expect(Cookie.new(type: 'Raisin').bake!(11).status).to eq(:ready)
      end
    end

    context "when baked for at least 12 minutes" do
      it "is `:burned`" do
        expect(Cookie.new(type: 'Raisin').bake!(13).status).to eq(:burned)
      end
    end
  end
end
